...
type DelegatableRoutingClass = (typeof DELEGATABLE_ROUTING_CLASSES)[number];
...
function isDelegatableRoutingClass(value: string): value is DelegatableRoutingClass {
...
function buildWorkerTask(params: {
  task: string;
  missionId: string;
  routingClass: string;
  sessionKey: string;
...
function buildCompactReceipt(missionId: string, workerId: string, routingClass: string): string {
