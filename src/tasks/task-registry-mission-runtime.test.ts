...
describe("task-registry mission runtime", () => {
  ...
  it("increments, clears, and prunes pending mission notifications", async () => {
    await withTaskTempDir(async () => {
      ...
      await waitForAssertion(() => {
        const store = loadSessionStore(storePath, { skipCache: true });
        expect(store["agent:main:main"]?.pendingMissionNotifications?.["mission-source-2"]).toBeTruthy();
      });
      ...
      await waitForAssertion(() => {
        const store = loadSessionStore(storePath, { skipCache: true });
        expect(store["agent:main:main"]?.pendingMissionNotifications?.["mission-source-3"]).toBeTruthy();
      });
